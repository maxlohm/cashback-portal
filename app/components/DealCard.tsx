...
type DealCardProps = {
  ...
  image: string;
  offerId: string;
  url: string;              // z.B. /angebot/<id>
  onClick?: () => void;
};

export default function DealCard({ name, description, reward, image, offerId, url, onClick }: DealCardProps) {
  const router = useRouter();
  const handleClick = () => {
    onClick?.();
    router.push(url);       // <— nur intern zur Detailseite
  };
  ...
  return (
    <div className="w-full md:w-[48%] flex flex-col items-center gap-4 p-6 bg-white rounded-xl border shadow hover:shadow-lg transition-all">
      <div className="flex items-center justify-center w-full bg-white p-2" style={{ height: 250 }}>
        <Image src={image} alt={`Bild zu ${name}`} width={300} height={250} className="object-contain rounded" />
      </div>

...
export default function InfluencerPage() {
  ...
  const { featured, rest, sumTotal, totalCount } = useMemo(() => {
    ...
    return { featured: featuredItems, rest: restItems, sumTotal: sum, totalCount: sorted.length }
  }, [rows, query, sort])
  ...
  return (
    <main className="mx-auto w-full max-w-6xl px-4 py-10">
      <div>
        ...
        <p className="mt-3 max-w-2xl text-base text-gray-600">Diese Creator teilen unsere Deals mit ihrer Community.</p>
      ...
      <div className="mt-8 flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-4 shadow-sm md:flex-row md:items-center md:justify-between">
        <input
          ...
          className="w-full rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm text-gray-900 outline-none focus:border-gray-300 md:flex-1"
        />
...
function Stat({ label, value }: { label: string; value?: number }) {
  return (
    <div className="rounded-xl bg-white px-3 py-2 text-center border border-gray-200">
      <p className="text-[11px] font-medium text-gray-500">{label}</p>
      <p className="mt-1 text-sm font-bold text-gray-900">{value == null ? '—' : formatNumber(value)}</p>
    </div>
  )
}

function socialBtn(kind: 'instagram' | 'tiktok' | 'whatsapp') {
  const base =
    'inline-flex items-center justify-center rounded-xl px-3 py-2 text-sm font-semibold border transition'

  if (kind === 'whatsapp') {
    return `${base} bg-[#25D366] border-[#1fb95a] text-white hover:brightness-95`
  }

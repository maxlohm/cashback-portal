...
export async function GET(req: Request, ctx: { params?: { offerId?: string } }) {
  ...
  // 4b) Sub-ID bestimmen:
  //     zuerst Sub-ID des Influencers (falls gesetzt), sonst die des Users
  ...
  if (ins.error) {
    if ((ins.error as any).code === '23505') {
      // duplicate -> update last click (solange nicht redeemed)
      mode = 'update'
  ...
  // 6) Den neuesten Click holen (für eventuellen clickToken)
  ...
  // 7) Ziel-URL bauen (Sub‑ID + Click‑Token werden zentral in buildAffiliateUrl gemappt)
  const dest =
    buildAffiliateUrl(offer.affiliate_url, {
      userId: user.id,
      offerId,
      influencerId,
      subId: partnerSubId || undefined, // 👈 Admin-SubID (profiles.partner_subid)
      clickToken: clickToken || undefined, // 👈 optionaler Click-Token
    }) || '/'
